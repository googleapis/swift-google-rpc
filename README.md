...
Add `swift-google-rpc` as a package dependency:

```bash
swift package add-dependency https://github.com/googleapis/swift-google-rpc.git --from 0.1.0-preview
```

Then add `GoogleRpc` to your target's dependencies:

```bash
swift package add-target-dependency GoogleRpc <target-name> --package swift-google-rpc
```
